...
def fix_start(s):
    ...
    """
    firstletter = s[0]
    rest = s.replace(firstletter, '*')

    return firstletter + rest[1:]
...
def not_bad(s):
    ...
    if 'not' and 'bad' in s and notloc < badloc:
            newsentence = s.replace(s[notloc:(badloc + 3)], 'good')
            return newsentence
    else:
        return s
